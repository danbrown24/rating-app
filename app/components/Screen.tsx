...
interface BaseScreenProps {
  ...
  /**
   * Pass-through for the onLayout event
   */
...
function useAutoPreset(props: AutoScreenProps) {
  ...
  function updateScrollState() {
    if (scrollViewHeight.current === null || scrollViewContentHeight.current === undefined) return;
...
export const Screen = observer(function Screen(props: ScreenProps) {
  const {
    ...
    keyboardOffset = 0,
    safeAreaEdges,
    StatusBarProps: StatusBarPropsOverride,
    statusBarStyle = "dark",
    onLayout,
  } = props;

  const rootStore = useStores();
  const statusBarProps = StatusBarPropsOverride || { style: rootStore.darkMode ? "light" : "dark" };
  const $containerInsets = useSafeAreaInsetsStyle(safeAreaEdges);

  return (
    <Container style={$containerInsets} onLayout={onLayout}>
